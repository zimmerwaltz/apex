...
// create PHP Server task
gulp.task('connectServer', function(){
  phpServer.server({base:'app', port:3000, keepalive:true});
});

//browser sync task
gulp.task('browserSync',['connectServer'], function() {
    browserSync.init({
...
gulp.task('disconnectServer', function() {
...
gulp.task('watchFiles',['browserSync','disconnectServer','sass'], function(){ //start browserSync and sass tasks concurrently before watch
   gulp.watch('app/scss/**/*.scss', ['sass']);  //watch scss files
   gulp.watch('app/*.html', browserSync.reload); //watch html files
   gulp.watch('app/**/*.php', browserSync.reload); //watch php files
